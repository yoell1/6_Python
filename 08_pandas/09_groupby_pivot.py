...
# two = (df['code']=='G0001') | (df['code']=='G0002')
...
float = (df.groupby(['sector','market'],as_index=False)['close'].mean())
print(float.columns.tolist())
...
"""
    ...
    df.pivot_table(index=행_기준_행 , columns=열_기준_열, values=집계대상_열, aggfunc=집계합수)
"""
d = df.copy()
...
#   df.melt(id_var=유지할_열, var_name=열이름을_담을_열, value_name=값을_담을_열)
